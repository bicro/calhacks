...
def poll_12_bit(filename, rate, samples,val_func, finish_func=None, bias=0):
	width=2
	channels=1
	data = []
	for _ in samples:
		data.append(val_func()-bias)
...
def tone_func(freq, rate, scale):
	def tgen():
		n = 0
		yield int(scale/2*sin(n*freq*2*pi/rate)+scale/2)
		n += 1
	g = tgen()
